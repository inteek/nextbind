...
namespace WCF.Servicios
{
    ...
    public class Ticket : ITicket<TicketsPorCliente, TicketsPorAgente, TicketsSupervisor, TicketsPorArea, TipoTicket>
    {
        ...
        public Response<Entidades.TipoTicket> ConsultaTipoTickets(string descripcion, string padre)
        {
            try
            {
                //int tempVal;
                //int? _id = Int32.TryParse(id, out  tempVal) ? Int32.Parse(id) : (int?)null;
                bool _padre = padre=="#" ? true : false;

                var objFramework = new Framework.Ticket();
                List<Framework.Libreria.ResultTipoTicket> lista = objFramework.ConsultaTipoTickets(descripcion, _padre);
                if(objFramework.Error == null)
                {

                    Response<Entidades.TipoTicket> result = new Response<Entidades.TipoTicket>();
                    result.List = objFramework.ConsultaTipoTickets(descripcion,_padre).Select(x => new Entidades.TipoTicket
                    {
                        idServicio = x.idServicio,
                        descripcionServicio = x.descripcionServicio,
                        claveServicio = x.claveServicio,
                        idServicioSuperior = x.idServicioSuperior,
                        servicioSuperior = x.servicioSuperior
                    }).ToList();
                    return result;
                }
                else
                {
                    ResponseError<Entidades.TipoTicket> result = new ResponseError<TipoTicket>(objFramework.Error);
                    return result;
                }
            }
            catch(Exception ex)
            {
                ResponseError<Entidades.TipoTicket> result = new ResponseError<TipoTicket>(ex);
                return result;
            }
        }

        public Response<int> ConsultaNoTicketMayor()
        {
            try
            {
                var objFramework = new Framework.Ticket();
                List<int> lista = objFramework.ConsultaNoTicketMayor();
                if(objFramework.Error==null)
                {
                    //Response<int> resultado = Response<int>();
                    //resultado = objFramework.ConsultaNoTicketMayor();
                    Response<int> resultado = new Response<int>();
                    resultado.List = objFramework.ConsultaNoTicketMayor();
                    return resultado;
                }
                else
                {
                    ResponseError<int> result = new ResponseError<int>(objFramework.Error);
                    return result;
                }
            ...
            {
                ResponseError<int> result = new ResponseError<int>(ex);
                return result;
            }
        }
    }
